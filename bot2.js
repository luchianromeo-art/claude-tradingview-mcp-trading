...
async function setLeverage(sym) {
  ...
  try {
    ...
    console.log(`[BOT2] MarginMode crossed ok ${sym}`);
  ...
  try {
    ...
    console.log(`[BOT2] Leverage 1x ok ${sym}`);
  } catch (e) { console.log(`Leverage skip ${sym}: ${e.message}`); }
}

async function setTpSl(sym, side, qty, tp, sl) {
  if (PAPER) return;
  // Incearca endpoint nativ BitGet — daca esueaza, fallback prin PnL% la fiecare run (fara alerta Telegram)
  try {
    ...
    console.log(`[BOT2] TP/SL nativ ok ${sym} TP=${tp} SL=${sl}`);
  } catch (e) {
    // Fallback silentios — PnL% verificat la fiecare run
    console.log(`[BOT2] TP/SL nativ skip ${sym} — fallback PnL% activ`);
  }
}
